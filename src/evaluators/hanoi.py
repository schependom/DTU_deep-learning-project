...
class HanoiEvaluator:
    ...
    def update_batch(self, batch: dict, preds: dict):
        ...
        ignore_id = self.eval_metadata.ignore_label_id if self.eval_metadata.ignore_label_id is not None else pad_id
        
        valid_mask = (labels != pad_id) & (labels != ignore_id)
        
        # Token-level accuracy
        correct_tokens = (pred_tokens == labels) & valid_mask
        self.total_token_correct += correct_tokens.sum().item()
        ...
        for i in range(labels.shape[0]):
            ...
            if seq_valid_mask.any():  # Only count sequences with at least one valid token
                seq_correct = (pred_tokens[i][seq_valid_mask] == labels[i][seq_valid_mask]).all()
                self.total_sequences += 1
                if seq_correct:
                    self.exact_match_sequences += 1
        
    def result(self, save_path: Optional[str] = None, rank: int = 0, 
               world_size: int = 1, group=None) -> Optional[dict]:
        ...
        if self.total_tokens > 0:
            token_accuracy = self.total_token_correct / self.total_tokens
        ...
        if self.total_sequences > 0:
            exact_accuracy = self.exact_match_sequences / self.total_sequences
        ...
        metrics[f"test/{dataset_name}/total_sequences"] = self.total_sequences
        metrics[f"test/{dataset_name}/total_tokens"] = self.total_tokens
